    ...
    public static void setStatusBarInvisible(RxAppCompatActivity context, boolean isShow) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (isShow) {
                ...
                setStatusBar(context, ResourcesUtil.getColor(R.color.base_colorTheme));
            } else {
...
//                activity.getWindow().setStatusBarColor(statusColor);
    ...
    public static int getStatusBarHeight() {
        int resourceId = BaseApplication.getInstance().getResources().getIdentifier("status_bar_height", "dimen",
                "android");
        return resourceId > 0 ? BaseApplication.getInstance().getResources().getDimensionPixelSize(resourceId) : 0;
    }
