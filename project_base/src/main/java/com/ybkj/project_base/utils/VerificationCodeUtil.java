package com.ybkj.project_base.utils;
...
public class VerificationCodeUtil {
    ...
    public void codeCountDown(TextView verificationCode) {
        disposable = Observable.interval(0, 1, TimeUnit.SECONDS).subscribeOn(Schedulers.io()).observeOn
            ...
            if (integer == 0) {
                verificationCode.setText(ResourcesUtil.getString(R.string.base_verification_code_get));
                verificationCode.setEnabled(true);
                verificationCode.setAlpha(1);
            } else {
                verificationCode.setText(String.format(ResourcesUtil.getString(R.string.base_verification_code_get_sub),
                        integer));
                verificationCode.setEnabled(false);
                verificationCode.setTextColor(ResourcesUtil.getColor(R.color.project_base_color_theme));
